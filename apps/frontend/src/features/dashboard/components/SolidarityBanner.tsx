...
export default function SolidarityBanner() {
  ...
  return (
    <motion.div 
      whileHover={{ y: -2 }}
      className="relative overflow-hidden bg-brand-teal rounded-[2rem] sm:rounded-[2.5rem] p-6 sm:p-10 lg:p-14 shadow-2xl shadow-brand-teal/20 group cursor-default"
    >
      {/* Premium background effects */}
      <div className="absolute top-0 right-0 w-[150%] sm:w-[500px] h-[500px] bg-brand-mint/5 rounded-full -translate-y-1/2 translate-x-1/2 blur-[100px] pointer-events-none group-hover:bg-brand-mint/10 transition-colors duration-700" />
      ...
      <div className="relative z-10 flex flex-col lg:flex-row items-center justify-between gap-8 sm:gap-12">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 sm:gap-6 shrink-0">
          <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-2xl sm:rounded-[2rem] bg-white/5 backdrop-blur-xl flex items-center justify-center border border-white/10 shadow-inner group-hover:border-brand-mint/30 transition-all duration-500">
            <Heart className="w-8 h-8 sm:w-10 sm:h-10 text-brand-mint fill-brand-mint/10 group-hover:scale-110 transition-transform duration-500" />
          </div>
